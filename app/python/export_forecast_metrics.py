...
def markdown_table(rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows:
        lines.append("| " + " | ".join(format_cell(row.get(column)) for column in TABLE_COLUMNS) + " |")
    lines.append("")
    return "\n".join(lines)
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    with path.open("w", encoding="utf-8", newline="") as file:
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({column: format_csv_cell(row.get(column)) for column in TABLE_COLUMNS})
