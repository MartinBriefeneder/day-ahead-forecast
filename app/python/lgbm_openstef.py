...
MODEL_NAME = "openstef-lgbm"
MODEL_FAMILY = "openstef-lgbm"
...
def create_lgbm_workflow(target: str, weather_features: tuple[str, ...] = WEATHER_FEATURES):
    from openstef_core.types import LeadTime, Q
    from openstef_models.models.forecasting.lgbm_forecaster import LGBMHyperParams
    from openstef_models.presets import ForecastingWorkflowConfig, create_forecasting_workflow

    config = ForecastingWorkflowConfig(
        model_id=f"{target}_openstef_lgbm",
        model="lgbm",
...
def write_run_files(output_dir: Path, payload: dict[str, Any], metadata: dict[str, Any]) -> Path:
    ...
    return write_comparison_plot(output_dir, payload, metadata)
...
def write_comparison_plot(output_dir: Path, payload: dict[str, Any], metadata: dict[str, Any]) -> Path:
    ...
    plot_path = timestamped_report_path(
        output_dir,
        f"{metadata['target']}-openstef-lgbm-comparison",
    ...
    if has_actual:
        fig.add_trace(
            go.Scatter(
                ...
                y=[point.get("actualKwh") for point in points],
                mode="lines",
    ...
    fig.update_layout(
        title=f"OpenSTEF {target_label} LightGBM Forecast" f"{' vs Actual' if has_actual else ''}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run a standalone OpenSTEF LightGBM forecast.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    log_step(
        ...
        f"forecast_end={format_utc(forecast_end)}"
    )
